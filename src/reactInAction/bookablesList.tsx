...
type BookableTypeKeys = keyof BookableType;
...
const BookablesList = () => {
  ...
  const getUniqueValues = (arr: BookableType[], key: BookableTypeKeys) => {
  ...
  const groups = getUniqueValues(data.bookables, "group").map((item) => ({
    label: item,
    value: item,
  }));
    ...
    <div className="flex">
        ...
        <div className="flex flex-col gap-y-1">
          {bookablesInGroup.map((b, i) => (
            <span
              ...
              className={[
                "w-fit",
                bookableIndex === i ? "bg-red-600" : undefined,
              ].join(" ")}
            >
      ...
      {bookable && (
        <div className="w-1/2">
          ...
          <span>
            <Checkbox onChange={toggleDetails}>Show Details</Checkbox>
          </span>
          <p>{bookable.notes}</p>
          {hasDetails && (
            <div>
